...
mutable struct MyExperimentalDrugCocktailContext <: MyAbstractContextModel
    ...
    S::Float64             # safety constraint (maximum allowable total dosage in mg/kg)
    bounds::Array{Float64,2}  # bounds array with dimensions (K, 2) where each row contains [min, max] for each drug
